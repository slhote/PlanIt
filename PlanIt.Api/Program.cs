...
builder.Services.AddDbContext<PlanItDbContext>(options =>
    options.UseNpgsql(
        ...
        // Registers Npgsql's vector(N) type mapping (Pgvector.EntityFrameworkCore) for the Similar Tasks semantic-embeddings tables .
...
// Python generation source -- typed HttpClient
...
// for later cross-cutting concerns.
...
builder.Services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme)
    .AddJwtBearer(options =>
    {
        ...
        // HS256 shared secret — a single service both mints and verifies tokens, so no asymmetric RS256 split is needed.
        ...
        // an "access_token" query-string param instead — the standard pattern for this, scoped to
        // only the hub path so it doesn't weaken normal Bearer-header validation for REST calls.
...
// else via ProjectMember404ResultHandler below. This is a deliberate, acknowledged exception to
// "repository access is service-layer-only" — see ProjectMemberAuthorizationHandler's own comment for the rationale.
...
// Global exception handling via IExceptionHandler classes. Typed handlers run in registration order;
// each returns false if the exception isn't its type, falling through to the next.
// Anything none of them catch gets a generic 500 ProblemDetails from AddProblemDetails() below.
...
// Single GET /health endpoint confirming the API is up and the DB is reachable, for Azure deployment health probes.
