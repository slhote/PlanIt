...
import { fetchFeature, fetchSimilarWorkItems, fetchWorkItem } from "../api/workItems";
...
export function useSimilarWorkItemsQuery(projectId: Guid | undefined, workItemId: Guid | undefined) {
  return useQuery({
    queryKey: ["similarWorkItems", workItemId],
    queryFn: () => fetchSimilarWorkItems(projectId as Guid, workItemId as Guid),
    enabled: !!projectId && !!workItemId,
  });
}
