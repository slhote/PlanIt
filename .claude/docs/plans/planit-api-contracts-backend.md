...
**Structure**: `main` ← `feature/api-contracts-backend` (integration branch) ← one short-lived branch per §8 step (or per small cluster of steps).

- **Integration branch**: `feature/api-contracts-backend`, branched from `main` once, at the start of implementation. Nothing implementing this subplan is ever branched directly from `main`, and nothing implementing this subplan is ever PR'd directly to `main`.
- **Per-step branches**: one branch per §8 step, branched from the *current tip of* `feature/api-contracts-backend` (not from `main`, not from another step's branch). Suggested names, matching §8's numbering:
